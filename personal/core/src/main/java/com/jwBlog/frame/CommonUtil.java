package com.jwBlog.frame;

import com.alibaba.fastjson.JSONObject;
import com.jwBlog.frame.constants.ErrorEnum;
import com.jwBlog.frame.exception.CommonJsonException;
...
public class CommonUtil {
	...
	public static void hasAllRequired(final JSONObject jsonObject, String requiredColumns) {
		if (!StringUtilsNew.isNullOrEmpty(requiredColumns)) {
			//验证字段非空
			String[] columns = requiredColumns.split(",");
			String missCol = "";
			for (String column : columns) {
				Object val = jsonObject.get(column.trim());
				if (StringUtilsNew.isNullOrEmpty(val)) {
					missCol += column + "  ";
				}
			}
			if (!StringUtilsNew.isNullOrEmpty(missCol)) {
				jsonObject.clear();
